'use strict';
const { sb, verifyToken, ok, err, preflight, parseEvent } = require('./utils/supabase');

async function getAdmin(client, headers) {
  const user = await verifyToken(client, headers.authorization);
  if (!user || user.role !== 'admin') return null;
  return user;
}

exports.handler = async (event) => {
  if (event.httpMethod === 'OPTIONS') return preflight();

  let client;
  try { client = sb(); } catch (e) { return err(e.message, 500); }

  const { method, query, body, headers } = parseEvent(event);
  const action = query.action || '';

  if (action === 'verify') {
    const admin = await getAdmin(client, headers);
    if (!admin) return err('Bukan admin', 403);
    return ok({ ok: true, username: admin.username });
  }

  const admin = await getAdmin(client, headers);
  if (!admin) return err('Akses ditolak', 403);

  /* ── STATS ── */
  if (method === 'GET' && action === 'stats') {
    const [{ count: users }, { count: orders }, { count: chats }] = await Promise.all([
      client.from('profiles').select('id', { count: 'exact', head: true }),
      client.from('orders').select('id', { count: 'exact', head: true }).eq('status','approved'),
      client.from('chat_messages').select('id', { count: 'exact', head: true }),
    ]);
    return ok({ users: users||0, orders: orders||0, chats: chats||0 });
  }

  /* ── USERS ── */
  if (method === 'GET' && action === 'users') {
    const { data } = await client.from('profiles').select('id,username,plan,role,xp,coins,level,is_banned,created_at,last_seen,telegram_id').order('created_at', { ascending: false }).limit(100);
    return ok({ users: data || [] });
  }

  if (method === 'POST' && action === 'edit_user') {
    const { user_id, plan, role, coins, xp } = body;
    const upd = { updated_at: new Date().toISOString() };
    if (plan)   upd.plan = plan;
    if (role)   upd.role = role;
    if (coins !== undefined) upd.coins = parseInt(coins) || 0;
    if (xp !== undefined)    upd.xp    = parseInt(xp)    || 0;
    await client.from('profiles').update(upd).eq('id', user_id);
    return ok({ ok: true });
  }

  if (method === 'POST' && action === 'grant_plan') {
    const { user_id, plan, days } = body;
    const expires = new Date(Date.now() + (parseInt(days)||30) * 86400000).toISOString();
    await client.from('profiles').update({ plan, plan_expires: expires }).eq('id', user_id);
    return ok({ ok: true });
  }

  if (method === 'POST' && action === 'add_coins') {
    const { user_id, amount } = body;
    const { data: p } = await client.from('profiles').select('coins').eq('id', user_id).single();
    await client.from('profiles').update({ coins: (p?.coins||0) + parseInt(amount||0) }).eq('id', user_id);
    return ok({ ok: true });
  }

  if (method === 'POST' && action === 'add_xp') {
    const { user_id, amount } = body;
    const { data: p } = await client.from('profiles').select('xp').eq('id', user_id).single();
    await client.from('profiles').update({ xp: (p?.xp||0) + parseInt(amount||0) }).eq('id', user_id);
    return ok({ ok: true });
  }

  if (method === 'POST' && action === 'ban_user') {
    await client.from('profiles').update({ is_banned: !!body.banned }).eq('id', body.user_id);
    return ok({ ok: true });
  }

  if (method === 'DELETE' && action === 'delete_user') {
    await client.from('profiles').delete().eq('id', query.user_id);
    await client.auth.admin.deleteUser(query.user_id).catch(() => {});
    return ok({ ok: true });
  }

  /* ── ORDERS ── */
  if (method === 'GET' && action === 'orders') {
    const { data } = await client.from('orders').select('*').order('created_at', { ascending: false }).limit(100);
    return ok({ orders: data || [] });
  }

  if (method === 'POST' && action === 'approve_order') {
    const { data: order } = await client.from('orders').select('*').eq('order_id', body.order_id).single();
    if (!order) return err('Order tidak ditemukan', 404);
    const days    = parseInt(order.duration) || 30;
    const expires = new Date(Date.now() + days * 86400000).toISOString();
    await client.from('orders').update({ status:'approved', activated_at: new Date().toISOString(), expires_at: expires }).eq('order_id', body.order_id);
    if (order.user_id) await client.from('profiles').update({ plan: order.plan, plan_expires: expires }).eq('id', order.user_id);
    return ok({ ok: true });
  }

  if (method === 'POST' && action === 'reject_order') {
    await client.from('orders').update({ status: 'rejected' }).eq('order_id', body.order_id);
    return ok({ ok: true });
  }

  /* ── BROADCAST ── */
  if (method === 'POST' && action === 'broadcast') {
    const { title, message, target, type = 'info' } = body;
    if (!title || !message) return err('Title dan message wajib diisi');
    let q = client.from('profiles').select('id');
    if (target === 'premium') q = q.eq('plan','premium');
    else if (target === 'vip') q = q.eq('plan','vip');
    else if (target === 'free') q = q.eq('plan','free');
    const { data: users } = await q;
    const notifs = (users||[]).map(u => ({ user_id: u.id, type, title, message, icon: '📢' }));
    if (notifs.length > 0) await client.from('notifications').insert(notifs);
    return ok({ ok: true, sent: notifs.length });
  }

  return err(`Action tidak dikenal: "${action}"`, 404);
};
