...
function sb() {
  ...
  if (!_client) {
    _client = createClient(SUPABASE_URL, SUPABASE_KEY, {
      ...
      db: { schema: 'public' },
      global: {
        headers: { 'X-Client-Info': 'kizai-server' }
      }
    });
  }
  return _client;
...
async function verifyToken(client, authHeader) {
  ...
  try {
    ...
    const { data: profile } = await client.from('profiles').select('id,username,avatar_color,avatar_emoji,telegram_id,bio,role,plan,plan_expires,xp,level,coins,streak,streak_last,games_played,chat_messages,tools_used,referral_code,referral_count,achievements,is_banned,preferences,last_seen,created_at,updated_at').eq('id', user.id).single();
...
setInterval(() => {
  const now = Date.now();
  for (const [k, v] of _limits) { if (now > v.reset) _limits.delete(k); }
}, 5 * 60 * 1000);
