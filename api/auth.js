...
module.exports = async (req, res) => {
  ...
  if (req.method === 'POST' && action === 'register') {
    ...
    // Buat user via signUp (tidak butuh service key)
    const { data: authData, error: createErr } = await client.auth.signUp({
    ...
    if (!authData?.user) return res.status(400).json({ error: 'Gagal membuat akun' });
  ...
  if (req.method === 'POST' && action === 'login') {
    ...
    if (!identifier.includes('@')) {
      ...
      // Get email from auth.users via admin (fallback: try login with username as email)
      try {
        const { data: u } = await client.auth.admin.getUserById(p.id);
        if (u?.user?.email) email = u.user.email;
        else return res.status(401).json({ error: 'Username atau password salah' });
      } catch {
