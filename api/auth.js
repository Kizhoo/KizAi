...
module.exports = async (req, res) => {
  ...
  if (req.method === 'POST' && action === 'login') {
    ...
    // Jika bukan email, cari email dari kolom email di profiles
    if (!identifier.includes('@')) {
      const uname = identifier.toLowerCase();
      // Coba dari kolom email langsung dulu
      ...
      // Prioritas: preferences.email (disimpan saat register)
      if (p.preferences?.email) {
        email = p.preferences.email;
      } else {
        // Fallback: admin API (butuh service key)
        try {
          const { data: u } = await client.auth.admin.getUserById(p.id);
          if (u?.user?.email) email = u.user.email;
          else return res.status(401).json({ error: 'Tidak bisa menemukan email. Coba login pakai email langsung.' });
        } catch {
          return res.status(401).json({ error: 'Tidak bisa menemukan email. Coba login pakai email kamu.' });
        }
      }
    }

    const { data, error } = await client.auth.signInWithPassword({ email: email.toLowerCase().trim(), password });
    if (error) return res.status(401).json({ error: 'Username/email atau password salah' });
