-- ════════════════════════════════════════════════════════════
--  KizAi v2 — Supabase Schema (FULL + IDEMPOTENT)
...
DROP POLICY IF EXISTS "sessions_all"        ON chat_sessions;
...
-- Leaderboard: allow reading public profile fields tanpa login
DROP POLICY IF EXISTS "profiles_leaderboard" ON profiles;
CREATE POLICY "profiles_leaderboard" ON profiles FOR SELECT
  USING (role = 'user');
...
CREATE POLICY "sessions_all" ON chat_sessions FOR ALL USING (user_id = auth.uid());
CREATE POLICY "messages_all" ON chat_messages FOR ALL USING (user_id = auth.uid());
...
BEGIN
  ...
  INSERT INTO profiles (id, username, referral_code, coins, avatar_emoji)
...
SELECT id, username, avatar_color, avatar_emoji, level, xp, coins, streak, plan,
  games_played, chat_messages, tools_used,
...
-- ════════════════════════════════════════════════════════════
--  SETUP CHECKLIST SETELAH JALANKAN SQL:
--
--  [ ] Supabase → Auth → Settings → MATIKAN "Enable email confirmations"
--
--  [ ] Vercel → kiz-ai → Settings → Environment Variables:
--        SUPABASE_URL         = https://xxx.supabase.co
--        SUPABASE_SERVICE_KEY = eyJ... (service_role key dari Settings > API)
--        SUPABASE_ANON_KEY    = eyJ... (anon key)
--        CF_ACCOUNT_ID        = (Cloudflare account ID untuk AI)
--        CF_API_TOKEN         = (Cloudflare AI API Token)
--        BOT_TOKEN            = (Token bot Telegram)
--        OWNER_ID             = (Telegram ID kamu sebagai owner)
--        WEB_URL              = https://kiz-ai.vercel.app
--        MIDTRANS_SERVER_KEY  = (opsional, jika pakai Midtrans)
...
--        3. Cari row kamu → ubah role = 'admin' → Save
