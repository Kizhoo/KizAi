...
router.all('*', async (req, res) => {
  ...
  if (req.method === 'POST' && action === 'login') {
    ...
    return res.json({
      access_token: sess.session.access_token,
      refresh_token: sess.session.refresh_token,
      user: { ...profile, email: sess.user.email, effective_plan: plan, accessible_models: accessibleModels(plan) }
    });
  }
  ...
  if (req.method === 'POST' && action === 'change_password') {
    ...
    if (!current_password || !new_password || new_password.length < 6) return res.status(400).json({ error: 'Password tidak valid' });
