...
router.all('*', async (req, res) => {
  ...
  if (req.method === 'POST' && action === 'register') {
    ...
    // ── STEP 1: Cek duplikat username + signUp secara paralel ──
    // Cek username dulu (cepat) sebelum signUp
    ...
    // ── STEP 2: signUp (1 round-trip ke Supabase Auth) ──
    const { data: sd, error: se } = await withTimeout(
      client.auth.signUp({ email: emailClean, password, options: { data: { username: uname } } }),
      12000, 'Registrasi timeout, coba lagi'
    ...
    if (se) {
      ...
      if (m.includes('timeout'))
        return res.status(503).json({ error: 'Server sibuk, tunggu 10 detik lalu coba lagi.' });
    ...
    const coins = referral_code.trim() ? 100 : 50;

    // ── STEP 3: Upsert profile (1 round-trip, HARUS sebelum auto-login) ──
    await client.from('profiles').upsert({
      id: uid, username: uname,
      telegram_id: (telegram_id||'').trim(),
      referral_code: ref, coins, xp: 0, level: 1, streak: 0,
      preferences: { email: emailClean, theme:'dark', accent:'blue', language:'id', fontSize:'md' }
    }, { onConflict: 'id' }).catch(()=>{});

    // ── STEP 4: Auto-login (1 round-trip) ──
    const { data: sess, error: le } = await withTimeout(
      client.auth.signInWithPassword({ email: emailClean, password }),
      12000, 'Login timeout'
    ).catch(e => ({ data: null, error: { message: e.message } }));

    // Background tasks — tidak perlu ditunggu
    Promise.allSettled([
      client.from('notifications').insert({
        user_id: uid, type: 'success',
        title: '🎉 Selamat datang di KizAi!',
        message: `Halo ${uname}! Akun berhasil dibuat. Kamu dapat ${coins} koin bonus!`,
        icon: '🎉'
      }),
      ...
      // Handle referral bonus di background
      referral_code.trim() ? client.from('profiles')
        ...
        .then(({ data: referrer }) => {
          ...
          client.from('referrals').insert({
            referrer_id: referrer.id, referred_id: uid, coins_given: 100
          }).catch(()=>{});
        }).catch(()=>{}) : Promise.resolve()
    ]);

    // Kalau auto-login gagal → suruh login manual (akun sudah terbuat)
    if (le || !sess?.session)
      return res.status(201).json({ needs_login: true, message: 'Akun dibuat! Silakan login.' });

    // Return langsung dengan data yang sudah kita tahu — tidak perlu SELECT profile lagi
    const plan = 'free';
    return res.status(201).json({
      ...
      user: {
        ...
        plan, role: 'user', coins, xp: 0, level: 1, streak: 0,
  ...
  if (req.method === 'POST' && action === 'login') {
    ...
    // Login (1 call) dengan timeout
    const { data: sess, error: le } = await withTimeout(
      client.auth.signInWithPassword({ email, password }),
      12000, 'Login timeout, coba lagi'
    ).catch(e => ({ data: null, error: { message: e.message } }));
    if (le) {
      ...
      if (le.message.toLowerCase().includes('timeout'))
        return res.status(503).json({ error: 'Server sibuk, tunggu beberapa detik lalu coba lagi.' });
  ...
  if (req.method === 'GET' && action === 'leaderboard') {
    ...
    let q = client.from('profiles')
      .select('username,avatar_emoji,avatar_color,plan,level,xp,chat_messages,games_played,streak,coins,last_seen');
    // Filter by period using last_seen as activity indicator
    if (period === 'weekly') {
      q = q.gte('last_seen', new Date(Date.now() - 7 * 86400000).toISOString());
    } else if (period === 'monthly') {
      q = q.gte('last_seen', new Date(Date.now() - 30 * 86400000).toISOString());
    }
    const { data } = await q.order(col, { ascending: false }).limit(limit);
