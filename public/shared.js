...
const KZ = {
  version: '4.0.0',
  name: 'KizAi',
  url: typeof window !== 'undefined' ? window.location.origin : 'https://kizhoo.my.id',
...
const Auth = {
  ...
  logout: () => {
    Store.del('token'); Store.del('user'); Store.del('refresh_token');
...
/* ═══════════════════════════════════════
   ...
   ═══════════════════════════════════════ */
const API = {
  async call(endpoint, options = {}, _retry = false) {
    ...
    try {
      ...
      try {
        ...
        const msg = status === 500 ? 'Server error' : status === 404 ? 'API tidak ditemukan' : `Server error (${status})`;
        console.error('[API Error]', status, text.slice(0, 300));
        return { error: msg, ok: false };
      }
      // Token expired - try to refresh
      if (res.status === 401 && !_retry) {
        const refreshed = await Auth.tryRefresh();
        if (refreshed) return API.call(endpoint, options, true);
        // Refresh failed - clear tokens and redirect to login
        Store.del('token'); Store.del('refresh_token'); Store.del('user');
...
/* ═══════════════════════════════════════
   ...
   ═══════════════════════════════════════ */
const Format = {
  number: (n) => new Intl.NumberFormat('id-ID').format(isNaN(n)||n==null?0:n),
...
function requirePlan(minPlan, featureName) {
  ...
  const upgradeLabel = `Upgrade ke ${planInfo.name} ${planInfo.icon}`;
  Modal.confirm(
    `Fitur ${planInfo.name}`,
  ...
  // Modal.confirm sets button text to '✓ Konfirmasi' — find the most recently added confirm modal and update its button
  const allModals = document.querySelectorAll('.modal-overlay');
  const lastModal = allModals[allModals.length - 1];
  if (lastModal) {
    const confirmBtn = lastModal.querySelector('[id$="-confirm"]');
    if (confirmBtn) confirmBtn.textContent = upgradeLabel;
  }
  return false;
}
...
const Notif = {
  ...
  async load() {
    ...
    const { data, ok } = await API.get('/api/auth?action=notifications');
    if (!ok) return;
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  Cmd.register([
    ...
    { group: 'Aksi', name: 'Upgrade Premium', icon: '⭐', desc: 'Mulai dari Rp 19.000', action: "location.href='/checkout?plan=premium'" },
