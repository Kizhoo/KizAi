  ...
  async call(endpoint, options = {}) {
    ...
    try {
      ...
      try {
        ...
        // Server return HTML error page (biasanya karena env vars belum diset)
        const status = res.status;
        const msg = status === 500 ? 'Server error — pastikan SUPABASE_URL & SUPABASE_SERVICE_KEY sudah diset di Vercel'
          : status === 404 ? 'API endpoint tidak ditemukan'
          : `Server error (${status})`;
...
const Notif = {
  ...
  async load() {
    ...
    if (!Auth.isLoggedIn()) return;
    const { data, ok } = await API.get('/.netlify/functions/auth?action=notifications');
    if (!ok) return;
  ...
  async read(id) {
    await API.post('/.netlify/functions/auth?action=read_notification', { id });
    this._unread = Math.max(0, this._unread - 1);
    this._updateBadge();
  },
  async readAll() {
    await API.post('/.netlify/functions/auth?action=read_all_notifications', {});
