...
// Self-ping setiap 10 menit agar Railway tidak sleep (Trial plan)
const SELF_URL = process.env.WEB_URL || 'http://localhost:' + (process.env.PORT || 8080);
setInterval(() => {
  require('http').get(SELF_URL + '/ping', (r) => {
    // keep alive
  }).on('error', () => {});
}, 10 * 60 * 1000); // 10 menit
