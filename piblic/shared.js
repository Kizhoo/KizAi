...
const KIZAI = {
  ...
  prefs: JSON.parse(localStorage.getItem('kz_prefs') || '{"theme":"dark","accent":"lime","fontSize":"md","customCursor":true,"reducedMotion":false,"soundEffects":false,"language":"id","compactView":false,"sidebarCollapsed":false}'),
  api:   '/api',
  version: '2.0.0',
};
...
// ────────────────────────────────────────────────────────────
function saveUser(token, user) { KIZAI.token = token; KIZAI.user = user; localStorage.setItem('kz_token', token); localStorage.setItem('kz_user', JSON.stringify(user)); }
function savePrefs() { localStorage.setItem('kz_prefs', JSON.stringify(KIZAI.prefs)); applyPrefs(); }
function applyPrefs() {
  const p = KIZAI.prefs;
  initTheme();
...
function logout() { localStorage.removeItem('kz_token'); localStorage.removeItem('kz_user'); location.href = '/auth.html'; }
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (KIZAI.token) {
    api('GET', '/api/auth?action=me').then(d => {
      if (d.user) { KIZAI.user = d.user; localStorage.setItem('kz_user', JSON.stringify(d.user)); updateUserUI(); }
    }).catch(() => {});
    loadNotifications();
  }
